...
/*
  // Search for gaming parts
  try {
    String url = "https://www.newegg.com/p/pl?d=gaming+pc+parts&N=4131";
    Document doc = Jsoup.connect(url).get();
    Elements items = doc.select("div.item-cell");

    System.out.println("\nHere are some gaming PC parts that fit your budget:");
    for (int i = 0; i < items.size(); i++) {
        String itemName = items.get(i).select("a.item-title").text();
        String itemPrice = items.get(i).select("strong.price").text();
        double itemPriceNum = Double.parseDouble(itemPrice.replace(",", ""));
        if (itemPriceNum <= budget) {
